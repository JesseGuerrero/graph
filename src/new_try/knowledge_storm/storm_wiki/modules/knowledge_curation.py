...
class StormKnowledgeCurationModule(KnowledgeCurationModule):
    ...
    def __init__(
        ...
        max_conv_turn: int,
        max_thread_num: int,
        x_retriever: Optional[Retriever] = None,
    ):
        """
        ...
        # X/Twitter-specific conversation simulator
        self.x_conv_simulator = None
        if x_retriever:
            self.x_conv_simulator = ConvSimulator(
                topic_expert_engine=conv_simulator_lm,
                question_asker_engine=question_asker_lm,
                retriever=x_retriever,
                max_search_queries_per_turn=max_search_queries_per_turn,
                search_top_k=search_top_k,
                max_turn=max_conv_turn,
            )
    ...
    def research(
        ...
        # Run X Investigator with X/Twitter-specific search if enabled
        if self.x_conv_simulator:
            x_persona = "X Investigator: Social media analyst who investigates public discourse, trending opinions, and real-time reactions on X/Twitter to understand how the topic is being discussed by the public."
            x_conversations = self._run_conversation(
                conv_simulator=self.x_conv_simulator,
                topic=topic,
                ground_truth_url=ground_truth_url,
                considered_personas=[x_persona],
                callback_handler=callback_handler,
            )
            conversations.extend(x_conversations)
