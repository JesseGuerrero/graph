...
class StormArticleGenerationModule(ArticleGenerationModule):
    ...
    def __init__(
        ...
        self.section_gen = ConvToSection(engine=self.article_gen_lm, retrieve_top_k=retrieve_top_k)
...
class ConvToSection(dspy.Module):
    ...
    def __init__(self, engine: Union[dspy.dsp.LM, dspy.dsp.HFModel], retrieve_top_k: int = 5):
        super().__init__()
    ...
    def forward(
        ...
        word_limit = 1500 * self.retrieve_top_k
        info = ArticleTextProcessing.limit_word_count_preserve_newline(info, word_limit)
...
class WriteSection(dspy.Signature):
    """Write a stakeholders report section based on the collected information. Make it in a narrative style with chronological significance for reporting. Keep it concise, easy to read and show only the most important information while keeping it short and easiest digestible.

    Here is the format of your writing:
        1. Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, "###" Title" to indicate subsubsection title, and so on.
        2. Use [1], [2], ..., [n] in line (for example, "The capital of the United States is Washington, D.C.[1][3]."). You DO NOT need to include a References or Sources section to list the sources at the end.
        3. IMPORTANT: Keep paragraphs short — 2-4 sentences maximum per paragraph. You MUST separate paragraphs with blank lines. Never write a paragraph longer than 4 sentences. Each paragraph should focus on one point or idea.
    """
