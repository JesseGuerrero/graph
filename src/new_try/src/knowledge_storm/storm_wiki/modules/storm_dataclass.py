...
class StormInformationTable(InformationTable):
    ...
    def collect_all_images(self) -> list:
        """Aggregate all images from Information.meta across all URLs, deduplicated by src."""
        seen = set()
        images = []
        for url, info in self.url_to_info.items():
            for img in info.meta.get("images", []):
                src = img.get("src", "")
                if src and src not in seen:
                    seen.add(src)
                    images.append({**img, "source_url": url})
        return images
    ...
    def retrieve_information(
        ...
        selected_urls = []
        selected_snippets = []
        if not self.collected_snippets or self.encoded_snippets.ndim < 2 or self.encoded_snippets.shape[0] == 0:
            return []
