...
class STORMWikiRunnerArguments:
    ...
    include_images: bool = field(
        default=False,
        metadata={"help": "If True, match and inject images from source pages into the article."},
    )
...
class STORMWikiRunner(Engine):
    ...
    def __init__(
        self, args: STORMWikiRunnerArguments, lm_configs: STORMWikiLMConfigs, rm, x_rm=None
    ):
        ...
        x_retriever = Retriever(rm=x_rm, max_thread=self.args.max_thread_num) if x_rm else None
    ...
    def _inject_images(self, information_table):
        """Match images from source pages to article sections and inject them."""
        import httpx
        import numpy as np
        from sentence_transformers import SentenceTransformer
        from sklearn.metrics.pairwise import cosine_similarity

        all_images = information_table.collect_all_images()
        if not all_images:
            return

        # Read the polished article (or draft)
        for fname in ["storm_gen_article_polished.txt", "storm_gen_article.txt"]:
            fpath = os.path.join(self.article_output_dir, fname)
        ...
        section_texts = [f"{h} {body}" for h, body in sections]
        section_embeddings = encoder.encode(section_texts)

        # Match: top-1 image per section, threshold 0.55
        sim_matrix = cosine_similarity(section_embeddings, image_embeddings)
        used_images = set()
        matches = {}  # section_idx -> image

        for sec_idx in range(len(sections)):
            scores = sim_matrix[sec_idx]
            best_idx = int(np.argmax(scores))
            if scores[best_idx] >= 0.55 and best_idx not in used_images:
                used_images.add(best_idx)
                matches[sec_idx] = all_images[best_idx]

        if not matches:
            return

        # Download matched images to .image_cache/
        cache_dir = os.path.join(self.article_output_dir, ".image_cache")
        ...
        manifest = {}
        for sec_idx, img in list(matches.items()):
            src = img["src"]
            url_hash = hashlib.sha256(src.encode()).hexdigest()[:16]
            ext = src.rsplit('.', 1)[-1].split('?')[0][:4].lower()
            ...
            if not os.path.exists(filepath):
                try:
                    ...
                    continue
            manifest[url_hash] = {
                "src": src, "alt": img.get("alt", ""), "filename": filename,
        ...
        # Save manifest
        Path(os.path.join(cache_dir, "manifest.json")).write_text(
            json.dumps(manifest, indent=2), encoding="utf-8"
        )

        # Inject image markdown into article after each matched section heading
        new_lines = []
        sec_idx = -1
        for line in lines:
            ...
            if line.strip().startswith('#'):
                sec_idx += 1
                if sec_idx in matches:
                    img = matches[sec_idx]
                    url_hash = hashlib.sha256(img["src"].encode()).hexdigest()[:16]
                    ext = img["src"].rsplit('.', 1)[-1].split('?')[0][:4].lower()
                    if ext not in ('jpg', 'jpeg', 'png', 'gif', 'webp'):
                        ext = 'jpg'
                    filename = f"{url_hash}.{ext}"
                    ...
                    if len(alt) > 80:
                        alt = alt[:80].strip() + '...'
                    new_lines.append(f'\n![{alt}](/api/articles/{self.article_dir_name}/images/{filename})\n')

        new_article = '\n'.join(new_lines)
        # Write back to polished article
        out_path = os.path.join(self.article_output_dir, "storm_gen_article_polished.txt")
        Path(out_path).write_text(new_article, encoding="utf-8")
    ...
    def run(
        ...
        self.topic = topic
        topic_slug = truncate_filename(
            topic.replace(" ", "_").replace("/", "_").rstrip(". ")
        )
        timestamp = str(int(time.time()))
        self.article_dir_name = f"{topic_slug}_{timestamp}"
        ...
        # image injection (after polishing to avoid LLM stripping image markdown)
        if self.args.include_images and information_table is not None:
            self._inject_images(information_table)
