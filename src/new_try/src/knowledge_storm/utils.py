...
class WebPageHelper:
    ...
    def download_webpage(self, url: str):
        try:
            res = self.httpx_client.get(url, timeout=15)
    ...
    def _extract_images_from_html(html: str, base_url: str) -> List[Dict]:
        ...
        skip_patterns = ('1x1', 'pixel', 'tracking', 'icon', 'logo', 'favicon',
                         'badge', 'avatar', 'emoji', 'spacer', '.svg', 'data:')
        images = []
        try:
            ...
            for img in soup.find_all('img', src=True):
                ...
                w = img.get('width', '')
                h = img.get('height', '')
                if (w and str(w).isdigit() and int(w) < 50) or (h and str(h).isdigit() and int(h) < 50):
                    continue
                ...
                # Get caption from figcaption or surrounding text
                caption = ''
                ...
                # Skip if no useful text at all
                text = f"{alt} {title} {caption}".strip()
                if len(text) < 5:
                    continue
                images.append({
                    'src': abs_src,
    ...
    def urls_to_articles(self, urls: List[str]) -> Dict:
        with concurrent.futures.ThreadPoolExecutor(
            ...
            results = list(executor.map(self.download_webpage, urls))

        articles = {}
        self.failed_urls = {}

        for (html, error), u in zip(results, urls):
            if html is None:
                self.failed_urls[u] = error or "Download failed"
                continue
            # Decode bytes to string, stripping replacement characters
            ...
            if article_text is not None and len(article_text) > self.min_char_count:
                articles[u] = {
                    "text": article_text,
                    "images": self._extract_images_from_html(html, u),
                }
            else:
                reason = "Empty content" if article_text is None else f"Too short ({len(article_text)} chars)"
                self.failed_urls[u] = reason
