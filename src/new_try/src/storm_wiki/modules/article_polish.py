...
class WriteLeadSection(dspy.Signature):
    """Write a lead section for the given stakeholders report with the following guidelines:
    ...
    2. The lead section MUST contain exactly 3-4 short paragraphs separated by blank lines. Each paragraph should be 2-4 sentences maximum. Never write a single long block of text.
    ...
    4. Keep the lead concise, easiest to read, containing the most important information in a narrative form chronologically as a summary of the report meant to lead into the rest of the report.
    """
...
class PolishPage(dspy.Signature):
    """You are a faithful report editor who removes repeated information, makes the grammer and sentence structure coherent, ensures it the writing is tight, concise, pops out with only its most useful information for improving understanding, keeps the content/narrative and separates paragraphs, sentences and prose in the dynamic manner of an expert non-fiction writer that writes tightly, easily and most digestibly. You won't delete any non-repeated content in the report. You will keep the inline citations and structure from the outline and section sequence (indicated by "#", "##", etc.). Here is the report:"""
