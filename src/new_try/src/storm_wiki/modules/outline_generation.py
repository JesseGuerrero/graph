...
class WriteOutline(dspy.Module):
    ...
    def forward(
        ...
        conv = "\n".join(
            [
                f"Consultant: {turn.user_utterance}\nExpert: {turn.agent_utterance}"
...
class WritePageOutline(dspy.Signature):
    """Write an outline for a stakeholders report. Make it in a narrative style with chronological significance for reporting. Keep it concise, easy to read and show only the most important information while keeping it short and easiest digestible.
    ...
    outline = dspy.OutputField(prefix="Write the stakeholders reporting outline:\n", format=str)
...
class WritePageOutlineFromConv(dspy.Signature):
    """Improve an outline for a stakeholders report. You already have a draft outline that covers the general information. Now you want to improve it based on the information learned from an information-seeking conversation to make it more concise and information valuable while being comprehensive enough for stakeholder decisions.
    Here is the format of your writing:
    1. Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, "###" Title" to indicate subsubsection title, and so on.
    2. Do not include other information.
    3. Do not include topic name itself in the outline.
    ...
    outline = dspy.OutputField(
        prefix='Write the stakeholders report outline (Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, ...):\n',
        format=str,
    )
